...
public final class SingularityPerformanceProbe {
    ...
    private static final int[] WORLDS = {1, 3, 4};
    private static final int[][] VIEWPORTS = {{960, 600}, {600, 400}, {1280, 800}};
    ...
    public static void main(String[] args) throws Exception {
        ...
        summary.add("IndustrialArt=" + com.bigphil.mergehell.render.IndustrialArt.class.getProtectionDomain().getCodeSource().getLocation());
        summary.add("probe=" + SingularityPerformanceProbe.class.getProtectionDomain().getCodeSource().getLocation());
        ...
        int closedCases = 0;
        for (int world : WORLDS) for (boolean high : new boolean[]{false,true}) for (int[] size : VIEWPORTS) {
        ...
        int expectedCases = WORLDS.length * 2 * VIEWPORTS.length;
        if (closedCases != expectedCases) throw new IllegalStateException("Incomplete benchmark matrix: " + closedCases);
        summary.add("closed_cases=" + closedCases + " expected_cases=" + expectedCases);
        summary.add("all_eighteen_mailboxes_closed_with_zero_retained_buffers=true");
        Files.write(output.resolve("samples.csv"), csv);
        Files.write(output.resolve("summary.txt"), summary);
        System.out.println(summary.get(summary.size()-4));
    }
}
