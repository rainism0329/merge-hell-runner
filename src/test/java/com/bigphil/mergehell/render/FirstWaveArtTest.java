...
class FirstWaveArtTest {
    ...
    @Test
    void attackMarkerFollowsActualEnemyTelegraphAndDisappearsAfterTheShot() {
        for (EntityType type : List.of(EntityType.CONFLICT, EntityType.LOCK)) {
            ...
            assertEquals(0, markerPixels(render(visuals.snapshot().enemies().get(0))));
            ...
            assertTrue(markerPixels(render(visuals.snapshot().enemies().get(0))) > 0);
    ...
    private static int markerPixels(BufferedImage image) {
        int count = 0;
        // The existing logical warning marker sits above the body and health bar.
